...
export function AddOrEdit({ data, close }: Props) {
  ...
  const form = useForm<<%= entityUpCase %>Model>({
    resolver: zodResolver(<%= entityUpCase %>Model.schema),
  });
  ...
  const onSubmit = async (values: z.infer<typeof <%= entityUpCase %>Model.schema>) => {
    console.log(values);
