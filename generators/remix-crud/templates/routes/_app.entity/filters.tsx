...
export function Filter() {
  ...
  const form = useForm<<%= entityUpCase %>FilterModel>({
    resolver: zodResolver(<%= entityUpCase %>FilterModel.schema),
    defaultValues: new <%= entityUpCase %>FilterModel(),
  });
  ...
  const mapQueryParamToForm = () => {
    ...
    searchParams.forEach((value, key) => {
      ...
      const formKey = toCamelCase(key) as keyof <%= entityUpCase %>FilterModel;
  ...
  const onClearFilters = () => {
    form.reset(new <%= entityUpCase %>FilterModel());
    navigate(``);
  };
